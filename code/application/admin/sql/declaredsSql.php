...
class declaredsSql
{
    ...
    public function getDeclaredsList($where,$sort,$order,$offset,$limit){
        $sql = sprintf('SELECT zsrm.*,zsrf.rf_title,zu.username,zu1.username AS username1 FROM zc_swj_report_month zsrm LEFT JOIN zc_swj_report_form zsrf ON zsrf.id=zsrm.rf_id LEFT JOIN zc_user zu ON zu.id=zsrm.op_user_id LEFT JOIN zc_user zu1 ON zu1.id = zsrm.chk_user_id  %s AND zsrf.rf_class = \'s\'  ORDER BY %s %s  LIMIT %d,%d',$where,$sort,$order,$offset,$limit);
        return $sql;
    }
    ...
    public function getDeclaredsRows(){
        $sql = sprintf('SELECT id,rf_title FROM zc_swj_report_form WHERE rf_class=\'s\'');
        return $sql;
    }

    //添加时 选择的录入人员姓名
    public function getDeclaredsRowss(){
